			...
			<?php
                ...
                foreach($transaction->readDataForCart($_SESSION['customerid'])[1] as $row)
				{
					if($row['fullQuantity'] != 0){
						...
						echo '<div class="col-4-lg subcategoryColor' . $num . ' product" id="' . $row['id'] . '">' . '<a href="singleProductPage.php?id='.$row['id'].'"><img class="productsImage" alt="' . $row['description'] . '" title="' . $row['description'] . '" src="data:image/jpeg;base64,' . base64_encode($row['image']) . '"width="100px"/></a> <p>' . $row['name'] . '</p> <p>' . $row['description'] . '</p> <p>$' . $row['cost'] . '.00</p> <a href="addToCart.php?id=' . $row['id'] . '">Add to Cart</a></div>';
