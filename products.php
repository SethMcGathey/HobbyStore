		...
		<div class="container-fluid" id="Not_Ajax_Output">
			...
			<?php
				...
				}else
				{
					...
					$num = 0;
					foreach ($pdo->query($sql) as $row) {
					    echo '<div class="col-4-lg subcategoryColor' . $num . ' product" id="' . $row['a.id'] . '">' . '<img src="data:image/jpeg;base64,' . base64_encode($row['b.image']) . '"width="100px"/> <p>' . $row['a.name'] . '</p> <p>' . $row['a.description'] . '</p> <p>' . $row['a.cost'] . '</p> <a href="addToCart.php?id=' . $row['a.id'] . '">Add to Cart</a></div>';
