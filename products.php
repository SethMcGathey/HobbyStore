	...
	<body>
		...
		<div class="container-fluid" id="Not_Ajax_Output">
			...
			<?php
				...
				if(isset($_GET['id']))
				{
					//$sql = 'SELECT * FROM product a JOIN subcategory d ON subcategory_id = d.id JOIN category c ON d.id = c.id JOIN image b ON a.id = b.product_id WHERE c.id = ' . $_GET['id'] ;
					//$sql = 'SELECT * FROM product a JOIN subcategory b ON subcategory_id = b.id JOIN category c ON b.id = c.id WHERE c.id = ' . $_GET['id'];
					$sql = 'SELECT a.id,a.name,a.cost,a.description,b.image FROM product a LEFT JOIN image b ON a.id = b.product_id WHERE a.subcategory_id = ' . $_POST["id"];
